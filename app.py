...
from models import Tag  # if not already imported
...
# Inside your group_detail route BEFORE return render_template(...)
    for t in group.tasks:
        try:
            t.created_plus2 = (t.created_at + timedelta(days=2)).strftime("%Y-%m-%d")
        except:
            t.created_plus2 = ""
...
@app.route('/groups/<int:group_id>/auto_assign_quick', methods=['POST'])
@login_required
def auto_assign_quick(group_id):
    group = Group.query.get_or_404(group_id)

    # Only group admin (you can remove this check if you want)
    if current_user.id != group.admin_id:
        return jsonify({"error": "Only group admin can quick-assign."}), 403
    ...
    if not detected_role:
        return jsonify({"error": "Could not determine role from text."}), 400

    # -------- Find members with that role --------
    members = GroupMembership.query.filter_by(group_id=group.id, role=detected_role).all()

    if not members:
        return jsonify({"error": f"No members with role '{detected_role}' in group."}), 404

    # -------- Choose member with minimum open tasks --------
    def open_task_count(user_id):
    ...
    try:
        chosen_mem = min(members, key=lambda m: open_task_count(m.user_id))
    except ValueError:
        return jsonify({"error": "No eligible members."}), 400

    assigned_user = User.query.get(chosen_mem.user_id)
    if not assigned_user:
        return jsonify({"error": "Assigned user not found."}), 500
    ...
    db.session.commit()       # commit so new_task.id becomes available

    # -------- Add tag (safe) --------
    try:
        from your_tag_file import get_or_create_tag     # adjust import
        role_tag = get_or_create_tag(detected_role)
        if role_tag:
            new_task.tags.append(role_tag)
        db.session.commit()
    except Exception as e:
        print("Tagging error:", e)
        db.session.rollback()

    # -------- Log activity with correct task_id --------
    try:
        activity = TaskActivity(task_id=new_task.id, message=f"Auto-assigned by {current_user.username}")
    ...
    notif = Notification(
        ...
        message=f"You were auto-assigned a task: {new_task.title}"
    )
    db.session.add(notif)
    db.session.commit()
    ...
    socketio.emit(
        "new_notification",
        {"user_id": assigned_user.id, "message": f"You were auto-assigned: {new_task.title}"},
...
def create_task(group_id):
    ...
    if form.validate_on_submit():
        ...
        if hasattr(form, 'tags') and form.tags.data:
            tags_csv = form.tags.data
        elif request.form.get('suggested_tags'):
            # fallback for AJAX/checkboxes: can be comma-separated or multiple form entries
            tags_csv = request.form.get('suggested_tags')

        if tags_csv:
            # allow comma-separated string
            if isinstance(tags_csv, str):
                tag_names = [t.strip().lower() for t in tags_csv.split(',') if t.strip()]
            else:
                # maybe a list
                tag_names = [t.strip().lower() for t in tags_csv]
            for tn in tag_names:
                tag_obj = get_or_create_tag(tn)
                if tag_obj:
